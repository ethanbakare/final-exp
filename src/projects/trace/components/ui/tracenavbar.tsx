...
export const TRNavbar: React.FC<TRNavbarProps> = ({
  ...
  // Determine if we're in morphing mode (idle/recording) or processing mode
  const isMorphingMode = state === 'idle' || state === 'recording';

  return (
    <div className={`trnavbar-container state-${state} ${className} ${styles.container}`}>
      {/* MORPHING MODE: Dual button trackers */}
      {isMorphingMode && (
        <>
      ...
      <style jsx>{`
        ...
        /* Processing states: no gap needed for single button */
        .state-processing_audio,
        .state-processing_image {
          gap: 0;
        }
