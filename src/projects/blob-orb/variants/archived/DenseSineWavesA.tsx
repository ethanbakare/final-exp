/**
 * ARCHIVED — Highlight-only approach was explored but we went with the
 * highlight + hue shift variant (CoralStone) instead.
 *
 * Variant: Dense Sine Waves A (Highlight Only)
 *
 * 24 high-frequency waves with bump normals. Fragment shader only
 * brightens peaks — never darkens valleys. Bump detail shown through
 * additive highlights and per-bump specular.
 */
import React, { useRef, useMemo } from "react";
import { useFrame } from "@react-three/fiber";
import * as THREE from "three";
import { bumpHighlightShader } from "../../shaders/bumpHighlightShader";
...
const DenseSineWavesA: React.FC<Props> = ({ audioData, config }) => {
  ...
  return (
    <mesh ref={meshRef}>
      ...
      <shaderMaterial
        ...
        fragmentShader={bumpHighlightShader}
      />
    </mesh>
  );
};

export default DenseSineWavesA;
