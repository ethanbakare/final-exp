...
const SphericalHarmonics = dynamic(
  () => import("../variants/archived/SphericalHarmonics"),
  { ssr: false }
);
const NoiseField = dynamic(() => import("../variants/archived/NoiseField"), {
  ssr: false,
});
const HybridNoiseEnvelope = dynamic(
  () => import("../variants/archived/HybridNoiseEnvelope"),
  { ssr: false }
);
...
const config = {
  ...DEFAULT_CONFIG,
  maxDisplacement: 0.18, // Slightly higher so deformations are visible at small size
};
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 pb-32 select-none">
      <h1 className="text-center text-lg font-semibold text-gray-700 mb-6">
        Orb Deformation Variants
      </h1>

      {/* 2x2 grid of 4 variants */}
      <div className="max-w-3xl mx-auto grid grid-cols-2 gap-5">
        <MiniScene label="1. Spherical Harmonics">
          <SphericalHarmonics audioData={audioData} config={config} />
        </MiniScene>
        <MiniScene label="2. Noise Field">
          <NoiseField audioData={audioData} config={config} />
        </MiniScene>
        <MiniScene label="3. Hybrid Noise + Envelope">
          <HybridNoiseEnvelope audioData={audioData} config={config} />
        </MiniScene>
        <MiniScene label="4. Layered Sine Waves">
