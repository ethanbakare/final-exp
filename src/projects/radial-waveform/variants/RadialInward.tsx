 ...
 */
export default function RadialInward(props: RadialWaveformProps) {
  const { radius, renderExtent } = props;
  ...
  // Inward variant: bars extend INWARD only — canvas only needs radius + padding.
  const effectiveRenderExtent = renderExtent ?? radius + 20;
    ...
    const draw = (now: number) => {
      ...
      for (let i = 0; i < barCount; i++) {
        ...
        const barLength = Math.min(
          p.minBarLength + value * (p.maxBarLength - p.minBarLength),
          maxSafeInward,
        );
