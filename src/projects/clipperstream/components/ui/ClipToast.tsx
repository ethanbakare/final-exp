...
interface UnformattedCopyToastProps extends ToastProps {
  text?: string;  // Default: "Copied unformatted text"
}
...
/* ============================================
   VPN TOAST - Shows VPN warning message
   Icon: CautionIcon (warning triangle) | Text: "Switch off VPN for transcription" | Close: SubCloseIcon
   ============================================ */

export const VpnToast: React.FC<VpnToastProps> = ({
...
/* ============================================
   UNFORMATTED COPY TOAST - Shows copy confirmation for unformatted text
   Icon: SubCopyIcon (same as CopyToast) | Text: "Copied unformatted text" | Close: SubCloseIcon
   Used when formatting fails but clipboard copy succeeds
   ============================================ */

export const UnformattedCopyToast: React.FC<UnformattedCopyToastProps> = ({
  text = 'Copied unformatted text',
  onClose,
  className = '',
  fullWidth = false
}) => {
...
interface ToastNotificationProps {
  isVisible: boolean;
  onDismiss: () => void;
  type?: 'copy' | 'audio' | 'error' | 'vpn' | 'unformatted-copy';
  text?: string;
  ...
  return (
    <>
      ...
      <div className={`toast-notification ${isAnimatedIn ? 'visible' : ''} ${className}`}>
        ...
        ) : type === 'unformatted-copy' ? (
          <UnformattedCopyToast text={text} onClose={handleClose} fullWidth />
        ) : type === 'error' ? (
          <ErrorToast text={text} onClose={handleClose} fullWidth />
        ) : type === 'vpn' ? (
          <VpnToast text={text} onClose={handleClose} fullWidth />
        ) : (
...
const ToastComponents = { CopyToast, AudioToast, ErrorToast, VpnToast, UnformattedCopyToast, ToastNotification };
